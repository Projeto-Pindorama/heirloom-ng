...
/*	from OpenSolaris "main.c	1.9	05/06/08 SMI"	*/
		...
		*tokend = '\0';
		...
		if (*(tokend = exttok(current, ":,")) == ',') {
		    ...
		    *tokend = '\0';
			...
			if (rtnval = malloc(sizeof(struct sevstr))) {
...
)
{
	...
	char		       *text = NULL;	/* Ptr to user's text */
	char		       *label = NULL;	/* Ptr to user's label */
	char		       *tag = NULL;	/* Ptr to user's tag */
	char		       *action = NULL;	/* Ptr to user's action str */
	char		       *sstr = NULL;	/* Ptr to -s (severity) arg */
	char		       *ustr = NULL;	/* Ptr to -u (subclass) arg */
	char		       *cstr = NULL;	/* Ptr to -c (class) arg */
	...
	snprintf(labelbuf, sizeof (labelbuf), "UX:%s", cmdname);
	...
	if (u_seen) {
	    ...
	    errflg = FALSE;
	    p = strcpy(malloc(strlen(ustr)+1), ustr);
	    if ((tokenptr = strtok(p, ",")) == NULL) errflg = TRUE;
	    else do {
	...
	if (s_seen) {
		...
		if (sev_info->keyword != NULL) severity = sev_info->value;
		    ...
		    found = FALSE;
		    if ((sevstrval = getenv(SEV_LEVEL)) != NULL) {
			sevval = malloc(strlen(sevstrval)+1);
